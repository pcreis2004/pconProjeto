...
void cria_pipeline(ThreadArgs *threadArg) {
    ...
    printf("Conseguiste sacar o pipe cá para dentro e declaraste as 4 threads\n");
        pthread_create(&thread_contrast, NULL, contrast, (void *)threadArg);
        //pthread_create(&thread_smooth, NULL, smooth, (void *)threadArg);
        //pthread_create(&thread_textured, NULL, texture, (void *)threadArg);
        //pthread_create(&thread_smooth, NULL, sepia, (void *)threadArg);
 ...
 for (int i = 0; i < threadArg->numImagens; i++) {
        threadArg->files[i] = NULL;
        threadArg->filesout[i] = NULL;
    }
...
int main(int argc, char *argv[]) {
   ...
   ThreadArgs *threadArg = malloc(sizeof(ThreadArgs));
    ...
    // Allocate memory for the arrays of pointers (inside the loop)
    for (int j = 0; j < numImagens; j++) {
        threadArg->files[j] = malloc(MAX_LENGTH * sizeof(char));
        threadArg->filesout[j] = malloc(MAX_LENGTH * sizeof(char));
        if (threadArg->files[j] == NULL || threadArg->filesout[j] == NULL) {
            fprintf(stderr, "Memory allocation failed for threadArg->files[%d] or threadArg->filesout[%d]\n", j, j);
            exit(EXIT_FAILURE);
        }
    }
